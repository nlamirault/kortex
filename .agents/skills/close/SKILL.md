...
Session-end counterpart to `/today`. Enforces Hard Rule 1 (update before ending):
refreshes the hot cache and verifies consistency so the next session starts warm.
Keeps the pre-synthesized state accurate — the paper's persistence principle.
