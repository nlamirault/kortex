...
**Procedures (ADR-0001)**
- [ ] Every `.agents/skills/*/SKILL.md` has a `## Purpose` section — INFO if missing
- [ ] Orphan patterns — `wiki/patterns/*.md` not in `wiki/index.md` under Patterns and not linked from any `skill-impact.md` row — INFO
- [ ] `!failure` log entries with ≥2 occurrences not yet promoted to a `wiki/patterns/` page — INFO: candidate for `/evolve`
- [ ] `wiki/skill-impact.md` rows whose `Motivation` links a `pattern:`/`gap:`/`decision:` that does not exist — WARNING
...
Operation 3 (Lint). Audits the **wiki** layer for drift/decay/contradiction. Paired
with `/evolve`, which audits the **procedures** layer. Its gate checks (broken
links, placeholders, orphans) are reused by `/evolve` as the `lint` half of the
acceptance gate. Provenance format mandated by
