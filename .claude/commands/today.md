...
Session Startup Protocol. Replaces 2–4k tokens of "where were we?" recap with a
structured briefing from the hot cache — the paper's principle that accumulated
state should be *pre-synthesized and always ready*, not re-derived each session.
Provenance format mandated by [[decision:adopt-wikiskill-evolution-loop]].
