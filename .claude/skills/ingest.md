...
Operation 1 (Ingest). Compiles raw sources into the synthesized wiki layer — the
paper's `raw → wiki` arc, run once per source so knowledge compounds instead of
being re-derived per query. Provenance format mandated by
