...
**MPP is an open protocol for machine-to-machine payments that revives the HTTP `402 Payment Required` status code so any client can pay as part of the HTTP exchange — no account, API key, or billing relationship set up in advance.** Co-authored by [Tempo](../projects/tempo.md) (Tempo Labs) and Stripe.
...
MPP is deliberately **rail-agnostic and currency-agnostic**. The same protocol surface works over blockchain rails (EVM, [Solana](../concepts/blockchain-layer-1.md), Stellar, Hedera, NEAR Intents, the Lightning Network, USDC stablecoin) and over traditional processors (Stripe, card networks, ACH, and Tempo's own L1). A separation between abstract *intents* and concrete *methods* keeps the core minimal while allowing each network to plug in its own settlement mechanics.
...
- **Transports** — JSON-RPC and [MCP](../concepts/model-context-protocol-mcp.md) as message-passing transports, wiring MPP into AI-agent infrastructure.
...
- [Tempo](../projects/tempo.md) — the Stripe/Paradigm L1 blockchain that co-authors and settles MPP
- [Agent Payments Protocol (AP2)](../concepts/agent-payments-protocol-ap2.md) — Google-led agentic payment protocol; AP2 signs mandates over A2A/MCP, MPP settles over HTTP 402
- [Universal Commerce Protocol (UCP)](../concepts/universal-commerce-protocol-ucp.md) — commerce data/cart standard; complementary layer to MPP's settlement
- [x402](../concepts/x402.md) — sibling HTTP 402 payment protocol; both revive 402 for machine settlement
- [Model Context Protocol (MCP)](../concepts/model-context-protocol-mcp.md) — used as an MPP transport
- [AI Protocols](../concepts/ai-protocols.md) — protocol landscape hub
- [MPP Specs](../sources/mpp-specs.md) — the specification source

## Open Questions

- How does MPP relate to **x402** (also an HTTP-402 agentic-payment scheme)? Both revive 402 — convergence or fragmentation? See [x402](../concepts/x402.md).
